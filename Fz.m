function [Fz] = Fz(Cd, d, V_rel_x, V_rel_z, V_rel_mag, dU, m, spin)

%Fz calculates the vertical force (i.e., perpendicular to the surface)

%these are global parameters
global rho rho_p pi mu g Saffman surf_angle;

Fd = -(pi/8)*(d^2)*V_rel_mag*rho*Cd*V_rel_z;  %the aerodynamic drag force
F_grav = -m*cos(surf_angle)*g*((rho_p-rho)/rho_p); %the gravitational force
...
Fmag = -(pi/8)*(d^3)*rho*Cl_omega*spin*V_rel_x; %the Magnus lift force due to particle rotation    
...
if (Saffman==false) %checking whether the Saffman force should be calculated, which is computationally intensive and has little effect on trajectories
    Fz = Fd + F_grav + Fmag; %the total force. In this case the Saffman force, which is generally very small, is neglected
else
    ...
    Fsaff = -1.615*d^2*J*sqrt(rho*mu*abs(dU))*V_rel_x; %the Saffman lift force due to the shearing flow
    Fz = Fd+F_grav+Fmag+Fsaff; %the total force including the Saffman force
